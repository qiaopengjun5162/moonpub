...
- `moonpub configure --evidence-dir <dir>` 只在显式传参时保存真实微信后台取证截图，固定文件名为 `wechat-draft-created.png`、`configure-headed.png`、`preview-sent.png`；它不能替代人工脱敏审查，提交前必须确认图片不含二维码、token、cookie、AppSecret、手机号、账号隐私或后台敏感 URL。
...
- 微信编辑器的 Vue 模型会忽略 JS 合成点击（`isTrusted=false`）：picker 类弹窗的"确认"按钮用 JS `.click()`/`dispatchEvent` 点了没反应（弹窗不关、设置不生效），脚本必须先取元素坐标（`getBoundingClientRect` + `scrollIntoView`），再由 Rust 侧 `page.click(Point)` 发 CDP 可信鼠标事件；`position:fixed` 弹窗 `offsetParent` 为 null，可见性判断要用 `getClientRects().length`。保存前必须先关闭残留弹窗（picker 未关会挡住保存按钮，导致整轮设置不落库）。
