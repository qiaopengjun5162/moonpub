...
async fn push_wechat_draft_cookie(
    ...
    let author = front
        ...
        .to_owned();
    let thumb = cfg.wechat_thumb_media_id.clone().unwrap_or_default();
    // Upload article-specific cover image if one was generated by ship.
    let cover_png = dir.join(format!("{slug}.cover.png"));
    let thumb = if cover_png.exists() {
        match upload_image_cookie(&token, &cookie_header, &cover_png) {
            Ok(id) => id,
            Err(e) => {
    ...
    let media_id = create_draft_cookie(&token, &cookie_header, &draft)?;
...
fn upload_image_cookie(token: &str, cookie: &str, image_path: &Path) -> Result<String, AppError> {
    ...
    let url = format!(
        "https://mp.weixin.qq.com/cgi-bin/filetransfer?token={token}&lang=zh_CN&f=json&ajax=1&action=upload_mass_image&scene=2"
    );
    let resp = cookie_agent()
        .post(&url)
        .set(
    ...
    if out.is_empty() {
        return Err(AppError::PushFailed {
            message: "upload_image: 响应缺少 url/content".to_owned(),
            ip_hint: None,
        });
    }
    Ok(out)
...
fn create_draft_cookie(token: &str, cookie: &str, draft_json: &str) -> Result<String, AppError> {
    ...
    let cover = first_image_url(&content).unwrap_or_default();
    let random = format!(
        "0.{}",
