...
| Markdown 渲染 / Block / Theme | `█████████░` 90% | 已能产出微信 HTML，解析、行内语法、普通段落与 fence block 渲染已拆分；正文主题增至 14 套，并支持首段导语、h4 小标题和带 caption 图片 |
...
| 测试 / CI / 审计 | `███████░░░` 75% | CI 绿，本地 `cargo fmt --all -- --check`、`cargo clippy --all-targets --all-features --tests --benches -- -D warnings`、`cargo nextest run --all-features` 通过，当前 197 tests passed；`cargo llvm-cov nextest --all-features --summary-only` 总行覆盖 58.16%，浏览器自动化覆盖不足 |
...
- `render` — Markdown → WeChat HTML + draft.json（Block 模板系统 + inline CSS）
  ...
  - 支持 `--author` / `--thumb` 覆盖
  - 支持 theme 配置（default/warm/dark/geek/paper/magazine/notebook/classic/forest/sunset/ocean/mono/editorial/zen），通过 Theme 系统注入 inline CSS
...
### Theme 系统
- `theme = "default"|"warm"|"dark"|"geek"|"paper"|"magazine"|"notebook"|"classic"|"forest"|"sunset"|"ocean"|"mono"|"editorial"|"zen"` in moonpub.toml
- `Theme::section_style()` 生成 section 级 inline CSS
- 普通 Markdown 标题、首段导语、段落、引用、分割线、带 caption 图片、表格、列表和三反引号代码块统一走微信兼容 inline CSS 排版
