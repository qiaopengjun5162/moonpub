...
正式工作流区域现在也会把契约变成可点击的安全入口：当前文章会先检查或预览，飞书和照片会先生成草稿与本地预览；`wechat-draft` 只提示边界，不会从首页直接推进到微信草稿。

这个工作区工作台现在也开始更像插件首页，而不只是状态面板。你也可以直接从里面继续点：
...
`检查当前文章状态` 打开的当前文章工作台现在也提供继续操作按钮：可以直接预览当前文章；当 HTML 已经存在时，可以执行 `layout-audit` 排版审计；当 `draft.json` 已经存在时，也可以显式选择推进到微信草稿。排版审计只检查本地 HTML，不触发微信 API；微信动作仍然不会自动触发，点击前会继续走发布前风险提示。
...
排版审计会调用 `moonpub --json layout-audit <html>`，只检查本地 HTML 的公众号编辑器兼容风险，不触发微信 API，也不会打开浏览器。这样这条链路在插件里不再只是“跑完一条命令”，而是开始变成一个更像工作流结果页的入口。
