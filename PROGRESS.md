...
| 文档 / 教程 / 对外介绍 | `█████████░` 88% | README、首版发布清单、发布计划和中文发布文章初稿已补齐，安装路径已转向 v0.4.1，仍需真实截图/录屏 |
...
下一步补首版展示素材：用 v0.4.1 release 二进制录制本地预览、封面、`status` / `check` 截图或短视频；随后做一次真实微信草稿回归，但不要把浏览器自动化宣传成无人值守发布。
