...
- 2026-07-12: **插件构建纳入 PR CI** — `.github/workflows/build.yml` 的 `test` job 现在会先执行 `obsidian-plugin` 的 `npm ci && npm run build`；PR #133 对应 GitHub Actions run `29199006304` 已实际通过新增的 `Build Obsidian plugin` 步骤以及 Rust `test`、`windows-smoke`，避免插件无法打包时仍可合并。
