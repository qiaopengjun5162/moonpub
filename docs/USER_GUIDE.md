...
`检查当前文章状态` 打开的当前文章工作台现在也提供继续操作按钮：可以直接预览当前文章；当 HTML 已经存在时，可以执行 `layout-audit` 排版审计；当 `draft.json` 已经存在时，也可以显式选择推进到微信草稿。排版审计只检查本地 HTML，不触发微信 API；审计结果弹窗里可以再手动打开 HTML 预览；微信动作仍然不会自动触发，点击前会继续走发布前风险提示。
