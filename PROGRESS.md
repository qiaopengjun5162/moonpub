...
- 2026-07-26: **后台自动化收尾加固：可信点击 + 残留弹窗防护** — 创作来源确认按钮改为作用域定位（从已选 radio 逐层向上找 picker）+ 脚本只取坐标、Rust 侧 `page.click` 发 CDP 可信鼠标事件（微信编辑器 Vue 模型忽略 `isTrusted=false` 的 JS 合成点击，点了没反应曾导致 picker 不关、设置不落库）；可见性判断对 `position:fixed` 弹窗改用 `getClientRects()`；`step_baocun` 保存前先关闭残留弹窗（picker 未关会挡住保存按钮导致整轮不落库）；原创步骤去掉"作者信息未就绪"误报（就绪检测在新声明弹窗不可靠，直接确认 + 校验拦截重试即可）。2026-07-26 真实《瓦尔登湖》ship（草稿 100011218）复核四项全部落库且全程无误报；`cargo nextest run --all-features` 382 tests passed。
- 2026-07-26: **后台配置"假成功"根治：状态优先 + 显式保存 + 重载复核** — 修复 `ship` / `configure` 日志全 ✅ 但后台原创 / 赞赏 / 留言实际未生效的问题。原创步骤改用真实弹窗容器 `.claim__original-dialog`（协议勾选在 `.original_agreement`、确定在 `.weui-desktop-dialog__ft`，均不在正文 `#js_original_edit_box` 内），等待作者信息（可见 `input.js_author` 非空）就绪再确认，校验拦截自动重试；赞赏改为声明原创后点设置行 `.js_reward_open` 开关并在赞赏弹窗内确定（原创弹窗内的赞赏区域默认 `display:none`，点隐藏开关无效）；留言改为以 `input.js_interaction_setting` 的 `checked` 为真实状态，确认按钮限定在含"留言"的可见弹窗内（旧版全局盲点"确定"曾误中原创弹窗）；新增 `step_baocun` 显式保存草稿（不保存设置不落库）和 `step_fucha` 保存后重载编辑器逐项复核落库状态（`#js_original_open` 可见性 / `.js_reward_open` 文本 / `js_interaction_setting.checked` / `.js_claim_source_selected`），重载后的页面状态是唯一成功标准。状态读取全部改为可见性感知（设置行未选中的块 `display:none` 但仍在 DOM，`textContent` 包含匹配会把隐藏模板文字当成状态）。2026-07-26 真实《瓦尔登湖》ship（草稿 100011146）复核输出：原创已落库 / 赞赏已落库（账户： 寻月隐君）/ 创作来源已落库（个人观点，仅供参考）/ 留言已落库，手机预览 ret=0；`cargo nextest run --all-features` 380 tests passed。工程经验库新增"后台配置的 ✅ 必须读落库状态，不能信点击日志"。
...
- 2026-07-14: **插件首页最小回归门禁** — 将首页上下文文案、首次建议步骤、工作区路径脱敏和单实例模态替换下沉到无 Obsidian 依赖的 `workflow-ui.ts`，新增 Node 内建测试覆盖 Markdown / 图片 / 空上下文、Inbox / Articles 路径和旧模态先关闭再打开新实例的顺序。`npm test` 先严格类型检查该纯模块，再运行 3 项回归；PR CI 与 tag release workflow 均改为 `npm ci && npm test && npm run build`。DOM 级确认窗口与结果工作台 E2E 仍是下一阶段工作。
