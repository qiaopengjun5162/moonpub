...
| `yichen-video-content` | 逐句作用拆解、标题诊断、文字洁癖、可模仿结构 | 未来 `draft-audit` / `polish` 的内容质量诊断参考 | 可吸收诊断框架，不吸收视频抓取或爆款模板承诺 |
...
`yichen-video-content` 的启发不是“去做短视频工具”，而是它把内容诊断拆得很细：

- 标题承诺了什么。
- 开头有没有独立 hook。
- 每句话在承担什么作用。
- 哪些句子是填充、重复、空洞排比或 AI 味。
- 结构是否失衡，转折是否生硬。
- 哪些点值得模仿，哪些点应该避开。

这可以映射到 MoonPub 未来的文章质量门：

```bash
moonpub draft-audit <article.md> --json
```

它应该只做本地文本诊断，不改原文、不推草稿、不调用微信 API。若后续接 AI，也必须把“建议”和“自动改写”分开：

- 默认只输出诊断报告。
- 显式 `polish` 才生成改写稿。
- 改写稿进入新的 draft，不覆盖原文。
- 诊断不承诺“爆款”，只帮助文章更清楚、更像真人写的。
...
- 多平台社媒抓取和视频剪辑工具。
- 把“爆款结构模板”包装成 MoonPub 的发布承诺。
- 让 Agent 自动按诊断建议覆盖用户原文。
...
3. 再评估 `draft-audit` 这类只读内容质量门：先做标题、开头、结构、AI 味和逐段作用诊断，不做自动覆盖。
4. 公众号归档只从单篇公开 URL 开始，不碰批量历史和指标。
5. 其它平台抓取、微信本地库、视频工作流只做研究，不进入 v0.4.x / v0.5 主线。
