...
- 2026-07-10：早些时候 `git ls-files docs/first-run-evidence/**/*.png docs/first-run-evidence/**/*.jpg docs/first-run-evidence/**/*.jpeg docs/first-run-evidence/**/*.webp` 无输出，说明首次体验证据目录当时尚未提交截图图片；但仓库中已有 `Context/assets/qrcode.png` 这类历史二维码资产，隐私 / 二维码审查仍需人工确认后再勾选。
...
- 2026-07-13：首次将 `v0.4.2` 指向当前 `main` 后，Linux ARM64 的 Unix archive smoke 正确发现了测试标题大小写与后续路径不一致的问题：`new "Archive Smoke"` 生成 `Archive-Smoke.md`，而 Linux/macOS 的路径写成了 `archive-smoke.md`。Release 未创建，未产生发布资产；已改为统一使用小写连字符 smoke 标题，并在本地大小写敏感路径以打包 release 二进制重新验证通过，待修复 PR 合并后重新触发 tag workflow。
- 2026-07-13：修复 PR #134 合并后，第二次 v0.4.2 tag workflow `29214942987` 通过插件构建、五个平台构建、Linux/macOS `.tar.gz` archive smoke 和 Windows `.zip` smoke，并创建公开 [v0.4.2 Release](https://github.com/qiaopengjun5162/moonpub/releases/tag/v0.4.2)。本机从 Release 下载 macOS ARM64 包后，SHA-256 校验通过，解压二进制完成 `--version`、`init`、`new`、`render`、`check` 无凭证 smoke。
