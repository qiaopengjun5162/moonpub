...
| 对外材料 | `█████████░` 86% | README、上手指南、发布文章、演示素材记录和清单已有；还缺真实截图/短录屏 |
...
2. 准备展示素材：本地预览 HTML、封面 HTML、`status` / `check` 输出已完成；下一步导出截图和短录屏。
3. 做真实微信草稿回归：用真实凭证验证 `login`、`push --render`、`configure --headed`，但不打印或提交凭据。
