...
If you mainly work inside Obsidian, the plugin entry is no longer just a few commands. The homepage now runs `moonpub doctor --json` for local readiness, `moonpub workflow-registry --json` for supported workflow contracts, `moonpub evidence-status --json` for v0.4.2 evidence gaps, `moonpub release-check --json` for the v0.4.2 release gate, and `moonpub workspace --json` for workspace-level status and suggested next steps. The workflow section also maps safe-start contracts to buttons for current-article preview, Feishu draft preview, and photo draft preview, while WeChat draft handoff stays explicit and manual. See [obsidian-plugin/README.md](obsidian-plugin/README.md).
