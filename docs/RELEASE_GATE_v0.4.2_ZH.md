...
- 2026-07-10：新增 `configure --evidence-dir` 后，已用真实微信公众号后台生成并人工检查微信回归三张脱敏截图：`wechat/wechat-draft-created.png`、`wechat/configure-headed.png`、`wechat/preview-sent.png`。`cargo run -- --json evidence-status` 现显示 `3/11 present, 8 missing`，首页、飞书、照片证据和全仓隐私 / 二维码人工审查仍未完成，因此 release 总门禁仍不能通过。
