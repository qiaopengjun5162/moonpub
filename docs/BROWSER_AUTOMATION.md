...
MoonPub 的浏览器自动化定位是“本地发布副驾驶”，不是无人值守发布机器人：

- 用户必须自己完成微信后台扫码登录；MoonPub 只复用本地浏览器会话。
- 不绕过验证码、平台审核、账号权限、内容风控或最终人工确认。
- 自动化只处理重复配置步骤，例如原创声明、赞赏、留言、创作来源和预览。
- 最终发布动作应默认保留给用户在微信后台确认。
- 微信后台 DOM、class、交互文案可能随时变化；自动化步骤应软失败，不影响 API 草稿推送主流程。

对外介绍时，推荐表述为：MoonPub 的稳定核心是 Markdown 渲染和微信官方 API 草稿推送；CDP 自动化是提高效率的实验性辅助能力。
