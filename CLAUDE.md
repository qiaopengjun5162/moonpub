...
| 创作来源 | ⚠    | 点击"未添加"后搜索选项并选择；找不到可选项时软失败跳过 |
| 合集     | ⏸    | 已禁用，不执行 |
...
**根因**: 弹窗文字检测（"声明类型"/"文字原创"/"无需声明"）过于宽泛，会把页面上已可见的 原创声明 设置行误判为弹窗；跳过逻辑导致选择步骤从未执行。
**修复**: 移除弹窗类型检测，改为点击"未添加"后直接搜索已知选项（个人观点，仅供参考 / 个人观点 / 转载 / 翻译 / 综合整理 / 其他），命中后点击确认；找不到可选项时再关闭弹窗并软失败跳过。同时删除 `cdp` 中不再使用的 `has_visible_text` 辅助函数。
**经验**: 不要通过页面上的静态文字判断弹窗类型，容易误判。应直接对可操作元素（选项文本）做选择，失败再回退。
